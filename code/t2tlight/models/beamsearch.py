...
def _beam_search_step(time, func, state, batch_size, beam_size, alpha, eos_id):
    ...
    arr = np.zeros((25417), dtype=np.float32)
    for i in range(100):
        arr[i] = 10.
    idf_penalty = tf.constant(arr)
    ...
    #idf penalty
  #  idf_penalty = idf_penalty + tf.zeros((batch_size,beam_size,25417))
    #step_log_probs = tf.cond( time < 2,lambda : step_log_probs - idf_penalty, lambda : step_log_probs)
